export class ButtonUtil {
  ...
  static dressUpButton(
    ...
    sprite.on("pointerdown", function (_pointer: Phaser.Input.Pointer) {
      if (disableFunc()) return;

      sprite.setTint(0x00bb00);
    });

    sprite.on("pointerout", function (_pointer: Phaser.Input.Pointer) {
      if (disableFunc()) return;

      sprite.clearTint();
    });

    sprite.on("pointerup", function (_pointer: Phaser.Input.Pointer) {
      if (disableFunc()) return;
    ...
    sprite.on("pointerover", function (_pointer: Phaser.Input.Pointer) {
      if (disableFunc()) return;
